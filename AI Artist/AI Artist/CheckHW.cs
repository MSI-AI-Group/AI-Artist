...
namespace AI_Artist_V2
{
    internal class CheckHW
    {
        ...
        public T GetValueFromWMIManagementObject<T>(string sName, ManagementObject _o)
        {
            try
            {
        ...
        public bool GetGPU_TRT()
        {
            ...
            foreach (string VGA in VGA_Caption)
            {
                ...
                {                  
                    ...
                    if (result != 0)
                    {
                        Logger.Log("Unable to initialize CUDA");
                        return false;
                    }
                    result = cuDeviceGetCount(out deviceCount);
                    if (result != 0)
                    {
                        Logger.Log("Unable to get the number of graphics cards");
                        return false;
                    }
                    try
                    {
                        ...
                        if (match_text.Success)
                        {
                            ...
                            if (num < 6)
                            {
                                Logger.Log("graphics cards total memory is " + num + ",TRT must have at least 6G VRAM");
                                return false;
                            }
                            else
                            {
                                Logger.Log("graphics cards total memory is " + num );
                            }
                        }
                        else
                        {
                            Logger.Log("graphics cards total memory:"+output);
                            return false;
                        }
                    ...
                    if (match.Success)
                    {
                        DataCenter.GPU_Model = match.Value;
                        int num = int.Parse(DataCenter.GPU_Model);
                        if (num < 4050)
                        {
                            Logger.Log("graphics cards is " + num + ",TRT must have at least RTX4050 series or above");
                            return false;
                        }
                    }
                    if (VGA.Contains("RTX"))
