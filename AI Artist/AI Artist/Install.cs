...
namespace AI_Artist_V2
{
	internal class Install
	{
		...
		public async void Check_Python_Git_SD()
		{
			DataCenter._MainWindow.Dispatcher.Invoke(new Action(() =>
			{
				DataCenter._MainWindow.Text_Load.Text = "Check package";
			}));


			if ((!Check_Python()) ||
				(!Check_Git()) ||
				((DataCenter.IsSupport_TRT == true) && ((!Check_CUDA()) || (!Check_C()))))
			{
				DataCenter._MainWindow.Dispatcher.Invoke(new Action(async () =>
				{
					DataCenter._MainWindow.Text_Load.Text = "Install fail";
		...
		#region Check Python
		private bool Check_Python()
		{
			return ExecuteCommand("python --version").Contains("Python 3.");
		}
		#endregion
		...
		{
			return ExecuteCommand("git --version").Contains("git version 2.");
		}
		#endregion

		#region Check CUDA
		private bool Check_CUDA()
		{
			return ExecuteCommand("nvcc --version").Contains("nvcc: NVIDIA");
		}
		#endregion

		#region Check C
		private bool Check_C()
		{
			if (Directory.Exists(DataCenter.TRT_Cpp))
			{
				return true;
			}
			else
		...
		private bool Install_SD()
		{
			...
			Logger.Log($"Stable Diffusion Web UI installed in {DataCenter.Path_SD}.");
			return true;
		}
		#endregion
